...
public class OllamaService {
    ...
    public OllamaService(
        ...
        // Configura el WebClient con la URL base del API de Ollama
        this.webClient = webClientBuilder.baseUrl(ollamaApiUrl).build();
        this.modelName = modelName;
    }

    /**
     * Genera un resumen basado en el contexto de los documentos y la query original.
     * @param documentos Lista de DocumentoDTO para el contexto RAG (los 5 primeros resultados).
     * @param query La búsqueda original del usuario.
     * @param incluirQuery Si se debe incluir la query en el prompt (la opción solicitada).
     * @return El resumen generado por el LLM en español.
     */
        ...
        try {
            ...
            String requestCuerpo = new ObjectMapper().writeValueAsString(
                java.util.Map.of(
                    ...
                    "options", java.util.Map.of("temperature", 0.1) // Baja temperatura para resúmenes ??
                )
            );
